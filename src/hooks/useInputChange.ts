...
const useInputChange = ({ initialInput, initialInputErrors, validateInput }: UseInputProps): UseInputReturn => {
  ...
  const handleInputBlur = (e: ChangeEvent<HTMLInputElement>) => {
    const { name, value} = e.target
    const parsedValue = typeof selectedInput[name] === 'number' ? parseFloat(value) : value      
    validateInput(name, parsedValue)
  }
